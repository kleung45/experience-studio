  ...
  return (
    <>
      <section className="relative flex min-h-[85vh] flex-col items-center justify-center pt-20 pb-10 overflow-hidden">
        <div className="relative z-20 mx-auto flex max-w-5xl flex-col items-center text-center gap-8 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full justify-center pt-4">
            <a href="#/work" className="h-12 min-w-[160px] rounded-full bg-primary px-8 text-base font-bold text-white transition hover:bg-primary/90 hover:scale-105 active:scale-95 shadow-lg shadow-primary/25 flex items-center justify-center">
              View Our Work
            </a>
            <a href="#/contact" className="h-12 min-w-[160px] rounded-full border border-slate-300 bg-white/60 px-8 text-base font-bold text-slate-800 backdrop-blur-sm transition hover:bg-white/80 hover:border-slate-400 active:scale-95 flex items-center justify-center">
              Start a Project
            </a>
          </div>
        </div>
        <div className="relative z-0 mt-16 w-full">
          <div className="mx-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-1 overflow-hidden">
            {galleryImages.map((src, index) => (
              <div key={index} className={`group relative aspect-square overflow-hidden bg-gray-200 ${index >= 6 && 'hidden lg:block'} ${index >= 7 && 'lg:hidden xl:block'}`}>
                <img alt={`Project thumbnail ${index + 1}`} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" src={src} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 transition-opacity group-hover:opacity-100 flex items-end p-2 text-white text-xs font-medium">Project {String.fromCharCode(65 + index)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-surface-light border-t border-surface-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <a className="group flex items-center gap-2 text-sm font-semibold text-primary hover:text-primary/80" href="#/services">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {expertise.map((item, index) => (
              <div key={index} className="group relative overflow-hidden rounded-2xl bg-white p-8 transition-all hover:shadow-xl hover:shadow-primary/5 hover:border-primary/50 border border-transparent hover:scale-105 flex flex-col items-start text-left">
                <div className="mb-6 flex size-16 sm:size-20 items-center justify-center rounded-xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white transition-colors duration-300">
                  <span className="material-symbols-outlined text-4xl sm:text-5xl">{item.icon}</span>
                </div>
                <h3 className="mb-2 text-xl font-bold text-slate-900 font-display">{item.title}</h3>
                <p className="text-sm text-text-secondary leading-relaxed line-clamp-2">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="order-2 md:order-1 relative aspect-video md:aspect-square lg:aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl">
              <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 hover:scale-105" style={{ backgroundImage: "url('../public/Alibaba.jpg')" }}>
              </div>
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
              <div className="absolute bottom-6 left-6 text-white">
                <p className="text-xs font-bold uppercase tracking-widest text-primary mb-1">Featured Project</p>
                <h3 className="text-2xl font-bold font-display">Lumina Exhibition Hall</h3>
                <p className="text-sm text-gray-300">Hong Kong Cultural Centre</p>
              </div>
            </div>
            <div className="order-1 md:order-2 flex flex-col justify-center gap-6">
              <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl font-display">Where Ideas Meet Reality</h2>
              ...
              <ul className="space-y-4 mt-2">
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-secondary">Narratives that guide users through physical spaces.</p>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <span className="material-symbols-outlined text-primary mt-1">check_circle</span>
                  <div>
                    ...
                    <p className="text-sm text-text-secondary">Invisible tech that enhances rather than distracts.</p>
                  </div>
                </li>
              </ul>
              <div className="pt-4">
                <a href="#/work" className="text-primary font-bold hover:underline decoration-2 underline-offset-4 inline-flex items-center gap-1">
